use music::{SpotifyKind, parse_spotify_url};

#[test]
fn parses_track_url() {
    let (kind, id) = parse_spotify_url(
        "https://open.spotify.com/track/6y0igZArWVi6Iz0rj35c1Y?si=abc",
    )
    .unwrap();
...
fn parses_album_url() {
    let (kind, id) =
        parse_spotify_url("https://open.spotify.com/album/1a2b3c").unwrap();
...
fn parses_playlist_url() {
    let (kind, id) =
        parse_spotify_url("https://open.spotify.com/playlist/xyz").unwrap();
