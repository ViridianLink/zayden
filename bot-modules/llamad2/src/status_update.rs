...
impl StatusUpdate {
    ...
    pub async fn presence_update(ctx: &Context, new: &Presence) {
        if new.user.id != LLAMA_USER || new.guild_id.is_none_or(|id| id != LLAMA_GUILD) {
            return;
        }

        match new.status {
            OnlineStatus::DoNotDisturb | OnlineStatus::Idle | OnlineStatus::Online => {
                ctx.set_activity(Some(ActivityData::custom(LLAMA_ONLINE_STATUS)))
            }
