...
use serenity::{
    all::{ExecuteWebhook, Http, Webhook},
...
impl WebhookLogger {
    pub async fn new(http: Arc<Http>) -> WebhookLogger {
        let bot_name = http
            .get_current_user()
            .await
            .map(|mut u| std::mem::take(&mut u.name))
            .unwrap_or(FixedString::<u8>::from_static_trunc("Default"));

        let error_webhook = Webhook::from_url(&http, ERROR_LOGS)
            .await
            .expect("URL is static");
        let normal_webhook = Webhook::from_url(&http, NORMAL_LOGS)
            .await
            .expect("URL is static");

        WebhookLogger(Arc::new(Inner {
            http,
            bot_name,
    ...
    pub async fn send_log(&self, level: tracing::Level, target: &str, message: String) {
        let inner = &self.0;
        let webook = match level {
            tracing::Level::ERROR => &inner.error_logs,
            tracing::Level::INFO | tracing::Level::WARN => &inner.normal_logs,
            _ => {
                return;
            }
        };

        let webhook_name = format!("{}-Webhook [{}] [{target}]", inner.bot_name, level.as_str());
