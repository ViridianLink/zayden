...
> **Intake note (2026-08-09, `95ad9efc`):** the tagged backlog read as **eight
> `in-review` findings and zero `open`**, and every one of the eight was already
> committed — the tree was clean. Reconciled against the tree, not the record:
> honeypot #2→`c1a9ee05`, #4/#6/#8/#9→`5a5bd682` (the batch), #5→`cf132327`,
> #7→`44b9e0e9`, #11→`95ad9efc`, CC-11→`00e06b7c`; and honeypot #10's
> `1f06667d` was corrected to **`f6ccd11b`** (amended commit, identical content)
> — the third time this workflow has caught a fix note naming a dead sha, after
> bot #4 and CC-4. The `open` markers still sitting in the *module* files are
> stale in the other direction and were **not** touched here: `bot` DS-1 and
> `family` DS-2 carry their own fix notes, and `suggestions` DS-1 is fixed in the
> tree (`reaction.rs` routes through a `review_action(pos, neg, thresholds)`
> function, so the flipped subtraction is gone). They are left for the human,
> since editing eleven markers is not this task.
>
> **What that left for selection, and the lesson it repeats.** With the tagged
> queue empty and the untagged list down to feature work, the pick came from a
> **residual** — [CC-11](_cross-cutting.md)'s, which had explicitly asked to be
> opened as its own finding and named the owner's decision it was waiting on.
> That is now [CC-12](_cross-cutting.md). **A residual paragraph inside a closed
> finding is a queue item that nobody can see**, which is the same failure mode
> as CC-10's residual predicting CC-11 in writing and changing nothing. When a
> task defers something, open it — the 2026-08-05 note said this about CC-5's
> footnote and it happened twice more.
>
> **And the pass's own gate found a second one.** `cargo test` is **red on
> `main`** — `music`'s `resolver_timeouts.rs` fails 1/695 — for a reason
> unrelated to any change under review. It went unnoticed because the two tasks
> before this one substituted `cargo test --workspace --no-run` for a real run.
> Recorded as [music #4](music.md). **A `--no-run` compile is not a test run**;
> when a throwaway Postgres is unavailable, say which suites did not execute
> rather than letting a compile stand in for the gate.
>
> **A note on the concurrent-review path, which fired for real this time.** The
> human reviewed and committed (`b1170b19`) **while this task was still running**
> — the path [operating principle #2](#operating-principles) describes — and used
> that commit to *narrow the scope*, keeping the `bacon` jobs and dropping the CI
> workflow the task had also written. Two things follow, both worth doing next
> time. **(1) An untracked new file is invisible to that path.** The workflow file
> was untracked, so it was not in the commit and was subsequently gone from the
> tree; a mid-run review captures *tracked modifications* and silently leaves new
> files behind. If a task's deliverable is a **new** file, say so explicitly at
> the pause rather than assuming the diff shows it. **(2) A scope cut has to be
> written into the finding, not just enacted in the tree.** The commit alone would
> have left `CLAUDE.md` and CC-12 both advertising a CI gate that does not exist —
> which is *precisely* the phantom-`sqlx-check.yml` defect CC-12 was opened to
> fix. The finding now records the ruling and names the residual it leaves.
...
| [music #4](music.md) — `resolver_timeouts.rs` paused clock + real socket → `cargo test` red on `main` | med | **New (2026-08-09, from CC-12's gate).** The mandated `cargo test` gate currently fails 1/695 for an ambient reason. Ranked above the CC-8 feature work: a red gate that everyone learns to ignore erodes every later task's evidence. Cheap — the fix is in the test harness, not the product. |
| [honeypot #10 follow-up](honeypot.md) — two coupling doc-comments | low | Dropped before `f6ccd11b` landed. Documentation-only; fold into whatever next touches `honeypot`. |
...
| ~~[honeypot #2](honeypot.md) — `GUARD.claim` non-atomic check-then-act~~ | med | **Done (`c1a9ee05`).** CC-9/double-submit shape on a `moka` cache. Fails-before was **221 winners across 128 floods**, not a marginal race. |
| ~~[`CC-11`](_cross-cutting.md) — `.sqlx` drifted again; `SQLX_OFFLINE` builds broken~~ | **high** | **Done (`00e06b7c`).** Marker reconciled 2026-08-09 against the tree (that commit renames the entry to `…86485e16…`, the hash the fix note names, and `.sqlx/` is at the stated 237). `docker/Dockerfile.bot` could not build; regenerated against an empty migrated DB. The finding's stated mechanism was **wrong and is corrected in place** — `c76ea93c` *did* regen, but the regen raced a reformat of the `.sql`, so cache and source were committed disagreeing over whitespace. Its follow-up — **nothing in CI checks this** — is now [`CC-12`](_cross-cutting.md). |
...
| ~~[honeypot #7](honeypot.md) — action path + authz gate untested~~ | med | **Done (`44b9e0e9`).** Marker reconciled 2026-08-09. Re-pinned first and found **narrower than recorded** — `tests/guard.rs` (with #2) and `tests/settings.rs` (with `cf132327`) had already closed two thirds. Delivered the authz half: `is_privileged` extracted so the decision is testable, and the gate **hoisted to one call site**, which makes deleting it a `dead_code` build failure. Fails-before reproduced by restoring the 3-call-site shape and deleting one gate: clippy clean, all suites green, undetected. Recorded [#10](honeypot.md) and [#11](honeypot.md) rather than folding them in. |
| ~~[honeypot #5](honeypot.md) — `/honeypot set` duplicates the dashboard form~~ | med | **Done (`cf132327`).** Marker reconciled 2026-08-09. Owner ruled **keep both surfaces, converge the write** — the reaction-roles #3 shape, against the finding's own "retire `set`/`disable`" recommendation (arming is setup, but *dis*arming is incident response). One owner in `honeypot::settings`; both editors call it. **It was not behaviour-neutral:** the two writers *disagreed*, and the dashboard's `parse_id` mapped garbage and blank alike to `None`, so a garbled channel id silently disarmed the trap and reported success. 10 tests; fails-before by mutation (4/10, then 1/10). |
| ~~[honeypot #10](honeypot.md) — config gate tests one permission bit, the other two sites test two~~ | low-med | **Done (`f6ccd11b`).** Confirmed reachable — the owner settled the question the repo could not: an interaction's computed `member.permissions` does **not** expand `Administrator`, so a server Administrator really was locked out. `is_privileged` now delegates to `policy::is_staff`. **Sha corrected 2026-08-09:** the fix note names `1f06667d`, which is not on `main`; the commit was amended into `f6ccd11b` (same 3 paths, same `src/` delta) — the [bot #4](bot.md) `12f6b01e`→`7e75cc79` precedent again. **Follow-up still open:** its two coupling doc-comments (on `is_privileged` and `is_staff`) were dropped before the commit landed. |
| ~~[honeypot #11](honeypot.md) — `message_create` ban/unban sequence has no test seam~~ | low-med | **Done (`95ad9efc`).** Marker reconciled 2026-08-09. The decision/effect split as prescribed: `is_decoy_hit` / `decide` / `outcome_of` extracted as pure covered functions, executor left a straight line. 7 tests, fails-before by 3 valid mutants. |
| ~~[honeypot #4](honeypot.md) — ban reason literal duplicated across two crates~~ | low-med | **Done (`5a5bd682`).** Marker reconciled 2026-08-09. Worked as a batch with #6/#8/#9 on the owner's instruction; `honeypot::BAN_REASON` is the single declaration and `bot`'s `HONEYPOT_REASON` is deleted. |
| ~~[honeypot #6](honeypot.md), [#8](honeypot.md), [#9](honeypot.md)~~ | low | **Done (`5a5bd682`)**, batched with #4; markers reconciled 2026-08-09. Dead `HoneypotHit.channel_id` deleted; both `GUARD.forget` calls **and** the method dropped; `PURGE_WINDOW` became `honeypot_settings.purge_seconds` (`migrations/0024_honeypot_purge`) with one clamp owner. |
| [`CC-12`](_cross-cutting.md) — nothing checks the `.sqlx` cache; the offline configuration is ungated | **high** | **`in-review` (`bacon` half landed `b1170b19`; docs uncommitted).** [CC-11](_cross-cutting.md)'s residual, opened on the owner's ruling of *offline check, not `prepare --check`*. Third record of this class in nine days, second broken image build. **Scope ruled down mid-task: `bacon` jobs + documented commands, no CI job** — so the blind spot is *narrowed, not removed*, and the mechanism that produced CC-10/CC-11 is still live. Recorded as CC-12's second residual rather than dressed up as a close. **Also fixed a doc defect that plausibly explains why the gap survived twice:** `CLAUDE.md` cited a CI workflow (`sqlx-check.yml`) that has never existed. |
