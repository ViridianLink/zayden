...
| [honeypot #10](honeypot.md) — config gate tests one permission bit, the other two sites test two | low-med | **New (2026-08-08, from #7's tests).** `is_privileged` ignores `Administrator` where `policy::is_staff` and the dashboard both honour it. Fails toward **lockout, not escalation**, which is why it went unseen. **Not confirmed** — reachability depends on whether Discord expands `Administrator` in an interaction's computed permissions, which the repo cannot settle. Check that first. |
| [honeypot #11](honeypot.md) — `message_create` ban/unban sequence has no test seam | low-med | **New (2026-08-08).** The half of #7 left undone: the parts are covered, the sequence and #1's outcome mapping are not. Needs a decision/effect split (serenity's `Http` is concrete, no trait seam) — a core-path refactor, so it is its own task. After #10. |
