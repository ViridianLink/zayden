...
| [palworld #2](palworld.md) — upload sweep unlinked on the reactor | low → **med in practice** | **`in-review` (2026-08-05).** The third untagged "confirm X" item, and the third to fail its confirmation. Its lesson: a confirm finding's *cited surface* is its weakest part. The audit sized this as "a single `remove_file`, usually tolerable"; the code had since moved to `remove_dir_all` on the uploader's whole directory (≤100 MB), on the reactor, every minute — and `bot/src/cron.rs` `join_all`s every job due in the same tick onto one task, so the stall was never confined to palworld. The finding's *other* half (`save/mod.rs`) confirmed clean exactly as predicted, so a failed confirm is not a reason to distrust the whole finding. Surfaced one new finding, [palworld #4](palworld.md) (`open`). |
| **[`CC-10`](_cross-cutting.md) — `.sqlx` cache drifted on `main`** | med | **`open` — recommended next, and a structural enabler.** Opened 2026-08-05 from a residual [CC-5](_cross-cutting.md) recorded on 2026-07-24 and never given its own finding. While `prepare --check` is red for a pre-existing reason, it cannot prove any *new* SQL change is sound — every SQL task since has hand-reverted unrelated drift. Needs a human-run migrated throwaway DB, so it is a **collaborative** task, not a solo agent one. |
| [palworld #4](palworld.md) — `stat`/`read_dir` on async command paths | low | **`open`.** Recorded 2026-08-05 by the palworld #2 close-out. Same class, far smaller: bounded metadata syscalls, but on user-facing `/pal` paths and on every call including cache hits, since the mtime *is* the cache key. |
