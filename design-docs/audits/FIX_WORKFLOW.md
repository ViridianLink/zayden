...
| [`CC-11`](_cross-cutting.md) — `.sqlx` drifted again; `SQLX_OFFLINE` builds broken | **high** | **Jump the queue.** `docker/Dockerfile.bot` cannot build *right now* — `c76ea93c` edited `winners.sql` without regenerating, one commit after CC-10's regen. Needs an empty migrated DB, so it is a collaborative task. |
| ~~[honeypot #3](honeypot.md) — dashboard settings write can violate the `guilds` FK~~ | med | **`in-review` (2026-08-07, uncommitted).** Fixed at the cross-cutting level as the finding asked: one `SettingsStore::seed_guild` owner covering all ten settings tables and both writers; honeypot's ad-hoc `INSERT` deleted. 4 tests, 3 fails-before. |
